...
(defmacro defttfconstant (name value &optional doc)
...
(defmacro defttfenum (name &body enum-list)
...
(defmacro defttftype (name base-type &optional documentation)
  `(eval-when (:compile-toplevel :load-toplevel :execute)
...
(defmacro defttfstruct (name &body fields)
...
(defmacro defttfunion (name &body fields)
...
(defmacro defttffun ((c-name lisp-name) return-type &body args)
  ...
  `(eval-when (:compile-toplevel :load-toplevel :execute)
     (cffi:defcfun (,c-name ,lisp-name :library libsdl2-ttf :convention :stdcall) ,return-type
       ,@args)
...
(defmacro defttf-lispfun (name lambda-list &body body)
  "Wrapper around `defun' which additionally exports the function name.
Meant to be used around SDL_ttf C preprocessor macros which have to be implemented as lisp functions."
...
(defmacro fdeclttf (struct-or-union)
